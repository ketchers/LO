...
class Chi2TestOfHomogeneityData(object):
    """
    This is the data for a chi-square homogeneity test. If initialized as

      ctx = Chi2TestOfHomogeneityData()
      
    then a default is used. The default is silly just to indicate what can be 
    done. There will be a few distributions with 4 outcomes. On each invocation
    a sample will be draw from randomply chosen distributions to fill out 
    ...
    def __init__(self, seed = None, **kwargs):
        ...
        row_dict = {'genders':['men', 'women'],
                    'ages':['under 25', '25 - 45', 'over 45']}
        ...
        null = "The %s preferences have the same distribution regardless of \
                %s ." % (self.col_name, self.row_name)
        
        alternative = "The distributions of %s preferences depends on the %s\
                       of those asked." % (self.col_name, self.row_name)
